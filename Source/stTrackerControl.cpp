...
stTrackerControl::stTrackerControl(BRect bounds, uint8 num_lines, uint8 speed,
	field_type *type_list, uint8 num_fields, BMessage *message, int32 resize)
	: BBox(bounds, "TrackerControl", resize, B_FRAME_EVENTS, B_NO_BORDER)
{
...
// ************ Set up the scroll view ***************'
	...
	bounds.OffsetTo(0, 0);
	bounds.bottom-=25;
	tracker_view = new stTrackerView(bounds, num_lines, type_list, num_fields,
		message);
	tracker_frame = tracker_view->Frame();

	AddChild((BView*)new BScrollView("tracker_scroll", tracker_view,
		B_FOLLOW_TOP_BOTTOM | B_FOLLOW_LEFT, 0, false, true));

	ResizeTo(tracker_frame.Width() + B_V_SCROLL_BAR_WIDTH,
		tracker_frame.Height() + 43.0);
...
// ************ Set up length gadget ***************'
	...
	length_string = new BTextControl(BRect(Bounds().left,
		tracker_frame.Height()+4, Bounds().right, tracker_frame.Height()+24),
...
// ************ Set up speed gadget ***************'
	...
	speed_string = new BTextControl(BRect(Bounds().left,
		tracker_frame.Height()+24, Bounds().right, tracker_frame.Height()+48),
