...
stInstrumentWindow::stInstrumentWindow(uint8 index, stMainWindow *main_win,
		BPoint point,Ins *instrument):
	BWindow(BRect(point,point+BPoint(385,425)),instrument->name,
...
// ********* set up the InsName ***************
	BTextControl * tc = new BTextControl("name","Name:",m_instrument->name,new BMessage(ST_INS_NAME));
	AddChild(tc);
...
// ********* set up the Tracker ***************
	...
	m_tracker_control = new stTrackerControl(m_instrument->len,
		m_instrument->sps, type_list, sizeof(type_list)>>2, new BMessage(43));

	BRect r;
	stTrackerEntry::PreferredSize(&r, type_list, sizeof(type_list)>>2);
	m_tracker_control->ChildAt(0)->SetExplicitMinSize(BSize(r.Width(), 16* r.Height()));
...
// ********* set up the loop textcontrol ***************
	...
	m_loop = new BTextControl("loop_textcontrol","loop",tmp,new BMessage(45));


// TODO layout all this properly!

	BGroupView* h = new BGroupView(B_HORIZONTAL, 3.0f);

	BGroupLayout* v = new BGroupLayout(B_VERTICAL);
	h->GetLayout()->AddItem(v);
	v->AddView(m_amplitude_envelope);
	v->AddView(m_filter_envelope);

	v = new BGroupLayout(B_VERTICAL, 0.0f);
	h->GetLayout()->AddItem(v);
	v->AddView(m_tracker_control);
	v->AddView(m_loop);

	AddChild(h);

	BGridView* sliders = new BGridView(0.0f, 0.0f);
	AddChild(sliders);
// ********* set up the resonance slider ***************
	...
	sliders->GridLayout()->AddView(view = new BStringView("string_view","resonance"), 0, 0);
	view->SetViewColor(216,216,216);
	view->SetLowColor(216,216,216);
	
	m_resonance = new BSlider("resonance_slider", 0, new BMessage(46), 0, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
...
// ********* set up the vibrato speed slider ***************
	...
	sliders->GridLayout()->AddView(view = new BStringView("string_view","vibrato speed"), 0, 1);
	view->SetViewColor(216,216,216);
	view->SetLowColor(216,216,216);
	
	m_vibrato_speed = new BSlider("vibrato_speed_slider", 0, new BMessage(47), 0, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
...
// ********* set up the vibrato depth slider ***************
	...
	sliders->GridLayout()->AddView(view = new BStringView("string_view","vibrato depth"), 0, 2);
	view->SetViewColor(216,216,216);
	view->SetLowColor(216,216,216);
	
	m_vibrato_depth = new BSlider("vibrato_depth_slider", 0, new BMessage(48), 0, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
...
// ********* set up the pwm speed slider ***************
	...
	sliders->GridLayout()->AddView(view = new BStringView("string_view","pwm speed"), 0, 3);
	view->SetViewColor(216,216,216);
	view->SetLowColor(216,216,216);

	m_pwm_speed = new BSlider("pwm_speed_slider", 0, new BMessage(49), 0, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
...
// ********* set up the pwm depth slider ***************
	...
	sliders->GridLayout()->AddView(view = new BStringView("string_view","pwm depth"), 0, 4);
	view->SetViewColor(216,216,216);
	view->SetLowColor(216,216,216);
	
	m_pwm_depth = new BSlider("pwm_depth_slider", 0, new BMessage(50), 0, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
...
//	m_pwm_depth->SetLimitLabels("0","255");
	...
	sliders->GridLayout()->AddView(m_resonance, 1, 0);
	sliders->GridLayout()->AddView(m_vibrato_speed, 1, 1);
	sliders->GridLayout()->AddView(m_vibrato_depth, 1, 2);
	sliders->GridLayout()->AddView(m_pwm_speed, 1, 3);
	sliders->GridLayout()->AddView(m_pwm_depth, 1, 4);
